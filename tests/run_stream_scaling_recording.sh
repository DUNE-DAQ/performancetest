...
if [ $# -ne 4 ]; then
  echo "Usage: ./run_link_scaling_recording.sh <envir_name> <run_num_init> <test_name> <server>"
...
do
	...
	nanorc ${i} $envir_name boot conf start_run $run_num expert_command --timeout 10 $test_name_${i}/$test_name_${i}/$readout record-cmd.json wait 300 stop_run shutdown
