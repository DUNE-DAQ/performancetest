...
for i in {8..40..8}
do
	global_cfg=np04daq-configs/global_configs/stream_scaling/np04_APA1_WIB_${i}_tp_perftest.json
	...
	echo "clean drive"
	rm /mnt/nvm_raid0/np04daq/output*.out
	sudo fstrim -v /mnt/nvm_raid0/

	nanorc $global_cfg $envir_name boot conf start $run_num pin_threads --pin-thread-file $cpupin_file enable_triggers expert_command --timeout 10 np04_hd/np04_daq/runp02srv003eth0 record-cmd.json wait 600 stop_run shutdown
